...
class Residual_Block(nn.Module):
    ...
    def __init__(self, inChannel, outChannel, stride=1, decSample=None):
        ...
        self.dropout = nn.Dropout(0.1) # 随机将张量中的部分元素置0，概率为参数对应的值
...
# ResNet50网络类
class ResNet50_2016(nn.Module):
    def __init__(self, layers, num_classes=6, model_path='model.pkl', loadModelFromPre=False, useGPU=True):
        super(ResNet50_2016, self).__init__()
        self.inChannel = 64
        self.modelPath = model_path
        self.useGPU = useGPU
        self.loadModelFromPre = loadModelFromPre
        self.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=True) # 与ResNet50_2012不同的地方在于一开始输入的通道数，这里是灰度图像，所以是1
